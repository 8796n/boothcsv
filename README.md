...
* **処理済み注文一覧の拡張**: 保存済み注文を一覧表示し、注文番号リンク・QR/画像アイコン・未印刷/未発送フィルタ・ページ移動・列ソートに対応しました
* **選択注文の再プレビュー改善**: 注文一覧から複数選択して印刷プレビューできるようになり、設定変更時も選択中の注文プレビューを再生成しやすくしました
* **印刷と編集の安定化**: 印刷完了後のスキップ面数がCSV再読み込み後も正しく反映されるよう修正し、カスタムラベルの右クリックメニュー位置ずれも改善しました
...
Windows版のChrome 146で動作確認をしています。  
最近のChromeなら動くと思いますが仕様が変わると動かない機能もあるかもしれません。  
スマートフォンのChromeだとラベルシール印刷がズレると思うのでたぶん駄目です。  
JavaScriptでどうにかしているので、**基本的な用途では別途サーバーなどは必要ありませんし、外部に情報を送ることもありません。**
...
* 注文明細の余白に画像を印刷したい場合は、サイドバーの「画像とカスタムラベル」内で「注文明細に画像を表示」にチェックを入れます。
* 全ての注文に同じ画像を表示したい場合は、サイドバー内のグローバル画像ドロップゾーンに画像をドラッグ&ドロップするかクリックしてファイル選択します。商品IDごとに画像を設定できますが、CSVの読み込み後に設定できるようになります。
...
これね、ヤマトの営業所持ち込みであんしんBOOTHパック使うときにはすげえ便利なんですよ。[PUDOステーション](https://booth.pixiv.help/hc/ja/articles/360013148033)に放り込むときにもめっちゃ便利です。慣れると1出荷15秒ぐらいでできます。
  ...
  補足: ラベルに表示される各注文番号は BOOTH の注文詳細ページ（例: `https://manage.booth.pm/orders/<注文番号>`）へのリンクになっています。リンク先であんしんBOOTHパック用のQRコードを生成・コピーできるため、該当注文のラベルから直接 QR 発行フローへ移動できます。
...
* なお間違えて貼り付けたときは**QRコードをクリック**すると再度貼り付けできるようになります。
...
2. **画像設定**: 以下の3つの方法で画像を設定できます：
   * **グローバル画像**: 全ての注文明細に同じ画像を表示したい場合。「注文明細の余白に表示したい画像をここにドロップ or クリックでファイルを選択」の部分に画像をドロップするか選択します。
   * **商品ID画像**: 商品IDごとに表示させる画像を切り替えたい場合に使用します。商品によって注意書きを変更したい場合などに使用できますが、複数商品を注文された場合は最初の商品IDの画像が表示されます。
...
個別画像は最優先、次に商品ID画像で、どちらも設定されていない場合にはグローバル画像が使用されます。つまり個別画像を設定した注文はその画像が、設定しなかった注文は商品ID画像かグローバル画像が表示されます。

画像とQRコード情報はブラウザのIndexedDBに内包保存されるので、ページを再読み込みしても残ります。不要になったらクリアボタンで削除できます：

* **全ての注文データをクリア**: 注文行・QR・個別画像をまとめて削除（グローバル画像/商品ID画像/フォント/設定は残る）
...
![処理済み注文一覧](docs/images/processed-orders-list.png)

CSVを読み込んで保存済み注文ができると、初期ガイドの下に処理済み注文一覧が表示されます。

* 注文番号を押すと、その注文だけの印刷プレビューを開けます
* 注文番号の右側には BOOTH 注文詳細へのリンクが付きます
* QRコードを保存済みの注文には QR アイコン、個別画像を保存済みの注文には画像アイコンが付きます
* 「未印刷のみ表示」「未発送のみ表示」で絞り込みできます
* 複数選択して印刷プレビュー、発送通知、削除ができます
...
* 別PCへ引き継ぐ場合は、バックアップJSONをエクスポートして移行先でリストアしてください
* 現在は IndexedDB を使ったローカル保存で、chrome.storage.sync のようなアカウント同期機能は実装していません
...
このリポジトリには、ローカルで簡単にファイルを配信するためのスクリプトが含まれています。Chrome から `boothcsv.html` を開くときに便利なもので、同じフォルダ内のファイルをホストします。

### 🪟 Windows の場合 (`boothcsv.ps1`)
...
#### 方法1: シェルスクリプト使用 (`boothcsv.sh`)

1. リポジトリのルート（`boothcsv.sh` があるフォルダ）に移動します。
2. 初回のみ実行権限を付与します:
...
# ブラウザを自動起動しない場合
./boothcsv.sh --no-browser
```

#### 方法2: Pythonのhttp.server使用（推奨）

macOSにはPython 3が標準でインストールされているため、最も簡単な方法です:

```bash
# ファイルがあるフォルダで実行
python3 -m http.server 8796

# ブラウザで http://localhost:8796 を開く
# 停止するには Ctrl+C
```
...
```bash
npx http-server -p 8796 -o
```

**PHP利用可能な場合:**

```bash
php -S localhost:8796
```

---
（補足）これらのスクリプトは簡易的な開発用ツールです。本番環境に公開する用途や外部からのアクセスを常時受ける用途には設計されていません。
